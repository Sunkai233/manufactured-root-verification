...
2. **认证 β=0 灾难性相消**:double 下 `β=|f(x0)/f'(x0)|` 在近根处相消恰好舍入成 0 → 假认证极病态实例(**旧 ∝1/γ 构造下** 2937 个;这是初值压地板的特有产物,当前两个无偏数据集分别为 0 和 2,见 criteria β=0 条件化一节);修法=认证升 long double(`mrv.hpp` 补 long double 重载否则 `derivs<long double>` 静默退化)。
